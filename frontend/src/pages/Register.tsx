...
export const Register: React.FC = () => {
  ...
  // Overlay states: 'register' | 'goal' | 'resources' | 'contact' | 'home' | null
  const [activeOverlay, setActiveOverlay] = useState<'register' | 'goal' | 'resources' | 'contact' | 'home' | null>('register');
        ...
        <AnimatePresence mode="wait">
          ...
          {activeOverlay === 'register' && (
            <motion.div
              ...
              className="w-full max-w-md p-8 bg-[#121418]/80 backdrop-blur-xl border border-[rgba(255,255,255,0.08)] rounded-3xl shadow-2xl relative mx-4"
            >
              <button 
              ...
              <div className="space-y-1 mb-6 text-center">
                ...
                <p className="text-[11px] text-zinc-500 font-medium">Join DockOps Control Console</p>
              </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold text-zinc-500 uppercase tracking-wider">First Name</label>
                    <input
                      type="text"
                      value={firstName}
                      onChange={(e) => setFirstName(e.target.value)}
                      placeholder="John"
                      className="w-full px-4 py-2 bg-[#0B0D10]/90 border border-[rgba(255,255,255,0.08)] rounded-xl text-xs text-zinc-200 placeholder-zinc-700 focus:outline-none"
                    />
                  </div>
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold text-zinc-500 uppercase tracking-wider">Last Name</label>
                    <input
                      type="text"
                      value={lastName}
                ...
                <div className="space-y-1">
                  <label className="text-[10px] font-bold text-zinc-500 uppercase tracking-wider">Platform Role</label>
                  <select
                    value={role}
                    onChange={(e) => setRole(e.target.value)}
                    className="w-full px-4 py-2 bg-[#0B0D10]/90 border border-[rgba(255,255,255,0.08)] rounded-xl text-xs text-zinc-300 focus:outline-none cursor-pointer"
                  >
                    <option value="DEVELOPER">Developer (Read/Operate Containers)</option>
                ...
                <button
                  ...
                  className="w-full py-2.5 px-4 bg-white hover:bg-zinc-200 disabled:bg-zinc-300 text-zinc-950 font-bold rounded-full text-xs transition-all flex items-center justify-center gap-2 cursor-pointer shadow-md mt-2"
                >
